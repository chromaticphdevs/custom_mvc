<?php
	...
	load(['Token'] , CORE);
 	...
 	abstract class Model extends ModelCore
	 {
		...
		public function _setDatabase($dbKey) {
			$databaseConfig = Config::database('uat');
			$this->db = new Database($databaseConfig['driver'], $databaseConfig['host'], $databaseConfig['dbname'], $databaseConfig['username'], $databaseConfig['password']);
			$this->dbHelper = new DatabaseHelper( Database::getInstance() );
		}
